...
### Analysing the Areas Where more the Speeding violations happens 
```{r}
#speedingPlot = scale(speedingPlot)

plot(longitude ~ latitude, data = t3_dat )
speedingPlot = t3_dat[t3_dat$speeding == "yes",c('longitude','latitude')]

num_clusters = 3
clust = kmeans(speedingPlot, num_clusters)
clusplot(speedingPlot, clust$cluster, color=TRUE,labels=0, lines=0)


mapLatitude = mean(clust$centers[,1])
mapLongitude = mean(clust$centers[,2])

map = get_map(c(-121.798305, 36.653753),maptype="road", zoom=16)
